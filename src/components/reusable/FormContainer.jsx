...
{/* 
           ...
           <Button onClick={()=>{setIsLoggedIn(true)}} type="submit" className="rounded-2xl bg-blue-500  lg:w-full text-white hover:bg-blue-600 hover:text-white">{!IsSignIn?'Create Acount':'Sign In'}</Button>
          <div className=' inline-block lg:hidden'>
          <span className="text-[13px] block  lg:hidden"><span  onClick={()=>{setIsSignIn(!IsSignIn)}} className="text-blue-500 cursor-pointer border-b border-blue-500">{IsSignIn ?'or create account':' or Sign In' }</span></span> 
          </div>
           </div>    
               ...
               {IsSignIn && (<a href='#' className='text-sm text-center cursor-pointer font-semibold'>forget password?</a>)}
                ...
                <div className=' lg:flex flex-col justify-between hidden '>
                   <span className="text-[13px] hidden lg:block"> {IsSignIn? 'Dont have an account yet':'Already have an account?'}<span  onClick={()=>{setIsSignIn(!IsSignIn)}} className="text-blue-500 cursor-pointer">{IsSignIn ?' create new for free!':' Sign In' }</span></span>
                    <img src="/signup.jpg" height={680} width={750}/>
                </div>
              </div>
</div>
            {!IsSignIn && (<span className=' right-2  text-center bottom-6  lg:bottom-10 lg:absolute items-end text-xs '>By signing up, you agree to our Terms & conditions, Privacy policy</span>)}
            </DialogContent>
          </Dialog>
  )
}
